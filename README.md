...
* Step Functions
* Fargate
  ![preview](./Images/aws_cloud237.png)
...
### AWS Lambda Integrations (Main ones)
  ![preview](./Images/aws_cloud238.png)
* **API Gateway:** Used to create a Rest API and they will invoke our Lambda Functions.
* **Kinesis:** Used to do some Data Transformations on the fly.
* **DynamoDB:** Used to create some Triggers, so whenever something happens in our DB a Lambda Function will be Triggered.
* **S3:** Lambda Function would be triggered anytime. For example, a file is created in S3.
* **CloudFront:** This will be Lambda at edge.
* **CloudWatch Events/EventBridge:** Whenever things happen in our infrastructure on AWS and we want to be able to react to things. For example, we have a code pipeline state changes and we want to do some automations based on it, we can use Lambda Function.
* **CloudWatch Logs:** To stream these logs whenever you want.
* **SNS:** To react to notifications in your SNS topic.
* **SQS:** To process messages from your SQS Queues.
* **Cognito:** To react to whenever. For example, user logins to your Database.

### Examples
* **Serverless Thumbnail Creation:**
  ![preview](./Images/aws_cloud239.png)
* **Serverless CRON Job:**
  ![preview](./Images/aws_cloud240.png)

### AWS Lambda Pricing: Example
* [Refer Here](https://aws.amazon.com/lambda/pricing/) to find Overall Pricing Information.
* Pay per **calls**:
...
* In `How it works`
  ...
  * It successfully runs, click `Next: Lambda responds to events`
    * Shows some Architectural View
  * Click `Next: Scale seamlessly`
    * It shows Architectural View with Costs
* Click `Create a function`
  * `Choose one options to create your function` (Use a blueprint)
  * Choose `Blueprint name` (Hello world function-python3.10)
  * Give `Function name`
  * Choose `Execution role` (Create a new role with basic Lambda permissions)
  * In `Lambda function code`, shows the Code
  * Click `Create function`
#### Hands-on
Go to Created Function.
* **Code:** This is a Code Editor
  * Click `Test` and click `Create new test event`
    * Give `Event Name` and click `Save`
    * Then again click `Test` and select `Event`
    * It shows the `Output`.
  * In Code Editor make some changes in Code
    * Click `Deploy`
    * Then click `Test`, select `Event` and shows the `Output`
* **Configuration:** Used to change the configuration
  * In Permissions, shows the IAM Role
* **Monitor:** Shows Metrics and Logs
...
* When you publish a new version:
  ...
  * Snapshot is cached for low-latency access
  ![preview](./Images/aws_cloud241.png)
...
* Native feature of CloudFront (manage code entirely within CloudFront)
  ![preview](./Images/aws_cloud242.png)
#### Lambda@Edge
...
* Author your functions in one AWS Region (us-east-1), then CloudFront replicates to its locations
  ![preview](./Images/aws_cloud243.png)
#### CloudFront Functions vs. Lambda@Edge
  ![preview](./Images/aws_cloud244.png)
...
* **Lambda by Default:**
  ...
  * Therefore, it cannot access resources in your VPC (RDS, ElastiCache, internal ELB…)
  ![preview](./Images/aws_cloud245.png)
* **Lambda in VPC:**
  * You must define the VPC ID, the Subnets and the Security Groups
  * Lambda will create an ENI (Elastic Network Interface) in your subnets
  ![preview](./Images/aws_cloud246.png)
...
* **The Lambda function must be deployed in your VPC, because RDS Proxy is never publicly accessible**
  ![preview](./Images/aws_cloud247.png)
...
* **DB instance must have the required permissions to invoke the Lambda function** (Lambda Resource-based Policy & IAM Policy)
  ![preview](./Images/aws_cloud248.png)
...
* Send notifications to SNS or subscribe to events using EventBridge
  ![preview](./Images/aws_cloud249.png)
...
### DynamoDB – Table example
  ![preview](./Images/aws_cloud250.png)
...
* In `Table details`,
  * Give `Table name` and `Partition key`
* Choose `Table settings` (Customize settings)
* Choose `Table class`
* In `Read/write capacity settings`, choose `Capacity mode` (Provisioned)
  * If Capacity Mode is Provisioned, provide `Read & Write capacity`
* In `Encryption at rest`, choose `Owned by Amazon DynamoDB`
* Enable `Deletion protection`, if required
* Click `Create table`.
#### Hands-on
* Go to created Table and click `Explore table items`
  * Click `Create item`
  * Give `Value` and click `Add new attribute`, give `Attribute name` and `Value`
  * After adding required attributes, click `Create item`.
* In this every Item has different Attributes.
...
* 5 minutes TTL for cache (default)
  ![preview](./Images/aws_cloud251.png)
* **DynamoDB Accelerator (DAX) vs. ElastiCache:**
  ![preview](./Images/aws_cloud252.png)
...
* **DynamoDB Streams:**
  ![preview](./Images/aws_cloud253.png)

### DynamoDB Global Tables
  ![preview](./Images/aws_cloud254.png)
...
* Use cases: reduce stored data by keeping only current items, adhere to regulatory obligations, web session handling…
  ![preview](./Images/aws_cloud255.png)
...
* **Export to S3 (must enable PITR):**
  ...
  * Export in DynamoDB JSON or ION format
  ![preview](./Images/aws_cloud256.png)
* **Import from S3:**
  * Import CSV, DynamoDB JSON or ION format
  * Doesn’t consume any write capacity
  * Creates a new table
  * Import errors are logged in CloudWatch Logs
  ![preview](./Images/aws_cloud257.png)

## AWS API Gateway
* **Example: Building a Serverless API**
  ![preview](./Images/aws_cloud258.png)
...
* **AWS Service Integration - Kinesis Data Streams Example:**
  ![preview](./Images/aws_cloud259.png)
...
### Create API Gateway
* Go to API Gateway
* `Choose an API type` (REST API) and click `Build`
* Select `API details` (New API)
* Give `API name`
* Choose `API endpoint type` (Regional)
* Click `Create API`
#### Create Method in Resource
* Go to created API and In `Methods`, click `Create method`
* Choose `Method type` (GET)
* Choose `Integration type` (Lambda function)
  * Go to Lambda and click `Create a function`
  * Choose `Author from scratch`
  * Give `Function name` and Select `Runtime` (Python)
  * Click `Create function`
  * Go to created Function and In `Code source`, provide below Code
    ```
    import json
  ...
  * Then Copy `Function ARN`
* Enable `Lambda proxy integration`
* In `Lambda function`, give `Function ARN` and select `Function`
* Click `Create method`
* **Test The Method:**
  * Go to Created Method and Navigate to `Test`
  * In `Test method`, click `Test`
  * The Method Result is shows below
#### Create Resource
* Go to Created API Gateway and click `Create resource`
* Give `Resource name` and click `Create resource`
* In that `Resource`, create a Method using Lambda
#### Deploy API
* Go to Created API Gateway and click `Deploy API`
* Select `Stage` (New Stage) and ive `Stage name`
* Click `Deploy`
* After Deploy API, we get `Invoke URL`
* Copy that `Invoke URL` and Browse in Web
  * It give the Root Data
  * We want to Print created Resource Data, give Resource Name at the End of the URL `/<Resource_Name>`
...
* **Use cases:** order fulfillment, data processing, web applications, any workflow
  ![preview](./Images/aws_cloud260.png)
...
* CUP integrates with **API Gateway** and **Application Load Balancer**
  ![preview](./Images/aws_cloud261.png)
...
### Cognito Identity Pools – Diagram
  ![preview](./Images/aws_cloud262.png)

### Cognito Identity Pools Row Level Security in DynamoDB
  ![preview](./Images/aws_cloud263.png)
...
* **Mobile App - REST API layer:**
  ![preview](./Images/aws_cloud264.png)
* **Mobile App - Giving Users Access to S3:**
  ![preview](./Images/aws_cloud265.png)
* **Mobile App - High Read Throughput, Static Data:**
  ![preview](./Images/aws_cloud266.png)
* **Mobile App - Caching at the API Gateway:**
  ![preview](./Images/aws_cloud267.png)
### In this lecture
...
* **Serving Static Content, Globally:**
  ![preview](./Images/aws_cloud268.png)
* **Serving Static Content, Globally, Securely:**
  ![preview](./Images/aws_cloud269.png)
* **Adding a Public Serverless REST API:**
  ![preview](./Images/aws_cloud270.png)
* **Leveraging DynamoDB Global Tables:**
  ![preview](./Images/aws_cloud271.png)
* **User Welcome Email Flow:**
  ![preview](./Images/aws_cloud272.png)
* **Thumbnail Generation Flow:**
  ![preview](./Images/aws_cloud273.png)
...
* **Micro Services Environment:**
  ![preview](./Images/aws_cloud274.png)
...
* **Our Application Current State:**
  ![preview](./Images/aws_cloud275.png)
* **Easy Way to Fix Things!:**
  ![preview](./Images/aws_cloud276.png)
### Why CloudFront?
...
* A popular **graph dataset** would be a **social network**
  ...
  * Users share and like posts…
  ![preview](./Images/aws_cloud277.png)
...
* Use cases:
  ...
  * Replicate data across regions in Neptune
  ![preview](./Images/aws_cloud278.png)
...
* **Immutable system:** no entry can be removed or modified, cryptographically verifiable
  ![preview](./Images/aws_cloud279.png)
...
* Use cases: IoT apps, operational applications, real-time analytics, …
  ![preview](./Images/aws_cloud280.png)
* **Amazon Timestream – Architecture:**
  ![preview](./Images/aws_cloud281.png)